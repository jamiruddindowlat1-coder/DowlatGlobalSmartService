...
export const AdProvider = ({ children }) => {
  ...
  return (
    <AdContext.Provider value={{ ads, addAd, removeAd, showAdmin, setShowAdmin }}>
      {children}
      {/* ✅ AdminModal শুধু showAdmin=true হলেই দেখাবে */}
...
export const AdminModal = ({ onClose }) => {
  ...
  const [form, setForm] = useState({
    ...
    link: "",
    body: "",
    imageUrl: "https://via.placeholder.com/300x150?text=Your+Ad+Here"
  });
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#1a3a5c", padding: "30px",
        maxWidth: "600px", width: '90%',
        borderRadius: "12px", border: '1px solid #27718f',
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <button type="submit" style={{ padding: '12px', background: '#27718f', border: 'none', color: '#fff', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}>
            Add Advertisement
          </button>
        </form>
        ...
        <div style={{ marginTop: '30px', maxHeight: '300px', overflowY: 'auto' }}>
          ...
          {ads.map(ad => (
            <div key={ad.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#132a44', padding: '10px', borderRadius: '6px', marginBottom: '8px' }}>
              ...
              <button onClick={() => removeAd(ad.id)} style={{ padding: '5px 10px', background: '#ff4d4d', border: 'none', color: '#fff', borderRadius: '4px', cursor: 'pointer', fontSize: '12px' }}>
                Delete
              </button>
            </div>
          ))}
...
// --- Ad Slot Component ---
// ✅ position:relative, zIndex নেই — লিঙ্ক block করবে না
export const AdSlot = ({ category }) => {
  ...
  return (
    <div style={{
      ...
      paddingTop: '15px',
      position: 'relative',
      zIndex: 1,          // ✅ খুব কম zIndex — লিঙ্কের নিচে থাকবে
      pointerEvents: 'auto',
    }}>
      <div style={{ fontSize: '10px', color: '#888', marginBottom: '8px', letterSpacing: '1px', textTransform: 'uppercase' }}>
        Sponsored
      </div>
      <a
        href={displayAd.link}
      ...
      >
        <div
          style={{ display: 'flex', gap: '15px', background: '#fdfdfd', border: '1px solid #efefef', borderRadius: '8px', padding: '12px', transition: 'transform 0.2s', boxShadow: '0 2px 5px rgba(0,0,0,0.02)' }}
